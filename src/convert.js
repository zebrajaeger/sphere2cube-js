...
exports.convert = (inX, outX, getPixel, setPixel) => {
...
function outImgToXYZ2(a, b, face) {
    // a:  [-1..1]
    // b: [-1..1]
    if (face === 0) {
        // back
...
exports.convert2 = (inX, edgeOut, face, getPixel, setPixel) => {
    const inSize = {x: inX, y: inX / 2, edge: inX / 4}
    ...
    for (let i = 0; i < edgeOut; ++i) {
        ...
        for (let j = 0; j < edgeOut; ++j) {
            ...
            const xyz = outImgToXYZ2(a, b, face)
            try {
                let rgba = calcPixel(xyz, inSize, getPixel);
                setPixel(i, j, rgba);
            } catch (err) {
